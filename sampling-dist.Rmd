...
the mean. We expect the distribution of water level to by asymmetric about its
mean. We check it by plotting the histogram with its empirical density.
```{r}
...
## Is the data normal?
In order to study the sampling distribution we first examine of the data is normal.
...
if (p.val > sig.p) {
  cat(paste(sep = "", "Data is normal.\n"))
  cat(paste(sep = "", "Shapiro-Wilk test has p-value ", p.val, " and significance level ", sig.p, ".\n"))
} else {
  cat(paste(sep = "", "Data is NOT normal.\n"))
  cat(paste(sep = "", "Shapiro-Wilk test has p-value ", p.val, " and significance level ", sig.p, ".\n"))
}
```
