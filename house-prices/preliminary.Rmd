...
```{r}
xyplot(SalePrice ~ MasVnrArea | MasVnrType, data = training.data)
```
...
```{r}
xyplot(SalePrice ~ BsmtFinSF1, data = training.data[training.data$BsmtFinSF1 != 0, ])
```

Basement area of type 1 influences sale price.

```{r}
xyplot(SalePrice ~ BsmtFinSF2, data = training.data[training.data$BsmtFinSF2 != 0, ])
```

Basement area of type 2 is not a very strong influencer of sale price.

```{r}
xyplot(SalePrice ~ BsmtUnfSF, data = training.data[training.data$BsmtUnfSF != 0, ])
```

Unfinished area of the basement is not a very strong influencer of sale price.

```{r}
xyplot(SalePrice ~ TotalBsmtSF, data = training.data[training.data$TotalBsmtSF != 0, ])
```
...
with(training.data[training.data$X2ndFlrSF != 0, ], plot(X2ndFlrSF, SalePrice))
with(training.data[training.data$LowQualFinSF != 0, ], plot(LowQualFinSF, SalePrice))
with(training.data, plot(GrLivArea, SalePrice))
```
