...
```{r}
bwplot(SalePrice ~ SaleCondition, data = training.data)
```
...
```{r}
bwplot(SalePrice ~ SaleType, data = training.data)
```
...
```{r}
bwplot(SalePrice ~ SaleType.1, data = temp.ds)
```
...
The distribution of data is
```{r}
bwplot(SalePrice ~ SaleType.2, data = temp.ds)
```
...
bwplot(SalePrice ~ MSSubClass, data = X)
rm(X)
```
...
```{r}
bwplot(SalePrice ~ MSZoning, data = training.data)
```
...
bwplot(training.data$LotFrontage,
       xlab = "Lot frontage",
       main = "Lot frontage")
```
...
type of lot shape.
```{r}
bwplot(SalePrice ~ LotShape, data = training.data)
```
...
Their box plot is
```{r}
bwplot(SalePrice ~ LandContour, data = training.data)
```
...
bwplot(Neighborhood ~ SalePrice, data = training.data)
```
...
bwplot(OverallQual ~ SalePrice, data = training.data)
```
...
bwplot(OverallCond ~ SalePrice, data = training.data)
```
...
bwplot(SalePrice ~ Age.1, data = age.data, xlab = "Age")
```
...
bwplot(SalePrice ~ YrSinceLastMod.1, data = age.data, xlab = "Age")
```
...
box plots.
```{r}
bwplot(MasVnrArea ~ MasVnrType, data = training.data)
```
...
```{r}
xyplot(SalePrice ~ X1stFlrSF, data = training.data, cex = 0.1)
xyplot(SalePrice ~ X2ndFlrSF, data = training.data[training.data$X2ndFlrSF != 0,], cex = 0.1)
xyplot(SalePrice ~ LowQualFinSF, data = training.data[training.data$LowQualFinSF != 0,], cex = 0.1)
xyplot(SalePrice ~ GrLivArea, data = training.data, cex = 0.1)
```
