...
\end{eqnarray*}
$I_1$ can be readily evaluated using the normalization of $p$. $I_2$ can be split as
\[
I_2 = I_{21} + I_{22} + I_{23},
\]
where
\begin{eqnarray*}
I_{21} &=& \frac{1}{2}\int_{\sor^n}(x - \mu)L^{-1}(x - \mu)p(x)dx \\
I_{22} &=& \int_{\sor^n}(x - \mu)^T\frac{L^{-1}}{2}(\mu - m)p(x)dx + \int_{\sor^n}(\mu - m)^T\frac{L^{-1}}{2}(x - \mu)p(x)dx \\
I_{23} &=& \frac{1}{2}\int_{\sor^n}(\mu - m)^TL^{-1}(\mu - m)p(x)dx.
\end{eqnarray*}
We got this split by writing $x - m = x - \mu + \mu + m$. It is easy to see that
\[
I_{23} = (\mu - m)^TL^{-1}(\mu - m)
\]
and $I_{22} = 0$.
\end{enumerate}
